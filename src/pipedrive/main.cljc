(ns pipedrive.main
  "Kotodama WASM entrypoint for the Pipedrive clean-room actor (L4) — Clojure port.
  ...
  facts (`pipedrive.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "pipedrive")
(def tier "L4")
...
(def entity-specs
  [{:entity "Deal"         :plural "deals"         :id-prefix "pipedriv_dea"
    :fields [:title :value :currency :status :probability :pipelineId :stageId] :required [:title :value]
    :coerce {:value :float :probability :float :pipelineId :int :stageId :int} :refs {:pipelineId "Pipeline" :stageId "Stage"}}
   {:entity "Person"       :plural "persons"       :id-prefix "pipedriv_per"
    :fields [:name :ownerId :orgId :marketingStatus] :required [:name :marketingStatus]
    :coerce {:ownerId :int :orgId :int}          :refs {:orgId "Organization"}}
   {:entity "Organization" :plural "organizations" :id-prefix "pipedriv_org"
    :fields [:name :ownerId]                       :required [:name]
    :coerce {:ownerId :int}                        :refs {}}
   {:entity "Pipeline"     :plural "pipelines"     :id-prefix "pipedriv_pip"
    :fields [:name :isDealProbabilityEnabled]      :required [:name :isDealProbabilityEnabled]
    :coerce {:isDealProbabilityEnabled :bool}      :refs {}}
   {:entity "Stage"        :plural "stages"        :id-prefix "pipedriv_sta"
    :fields [:name :pipelineId :dealProbability]   :required [:name :dealProbability]
    :coerce {:pipelineId :int :dealProbability :int} :refs {:pipelineId "Pipeline"}}
   {:entity "Activity"     :plural "activities"    :id-prefix "pipedriv_act"
    :fields [:subject :type :done :dueDate]        :required [:subject :type]
...
(defn emit-facts
  "EAVT facts for one record: {\"pipedrive.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "pipedrive-compat" :tier tier :entities entities} 200])
...
(defn start! [] :pipedrive-compat/ready)
